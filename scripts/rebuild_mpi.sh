...
# Rebuild just the MPI wrapper
echo "Rebuilding MPI wrapper..."
cd build
make mpi_wrapper
cd ..

echo "MPI wrapper rebuilt. Running the MPI test..."

# Set PYTHONPATH
export PYTHONPATH=$PYTHONPATH:$(pwd)/build/python

# Run the test with specified number of processes
NUM_PROCESSES=${1:-4}  # Default to 4 if not provided

echo "Testing MPI gradient synchronization with $NUM_PROCESSES processes..."
